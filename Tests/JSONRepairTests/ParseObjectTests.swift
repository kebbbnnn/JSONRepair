...
final class ParseObjectTests: XCTestCase {
    ...
    func testParseObject() throws {
        ...
        XCTAssertEqual(try JSONRepair.repairJson("{ \"key\": \"value\", \"key2\": 1, \"key3\": true }"), JSONRepair.convertToJSONValue([
            "key": "value",
            "key2": 1,
            "key3": true
        ]))
        XCTAssertEqual(try JSONRepair.repairJson("{"), JSONRepair.convertToJSONValue([String: Any]()))
        XCTAssertEqual(try JSONRepair.repairJson("{ \"key\": value, \"key2\": 1 \"key3\": null }"), JSONRepair.convertToJSONValue([
            "key": "value",
            "key2": 1,
            "key3": NSNull()
        ]))
    ...
    func testEdge_fooArray() throws {
        XCTAssertEqual(try JSONRepair.repairJson("{foo: [}"), JSONRepair.convertToJSONValue(["foo": [Any]()]))
    }
    
    func testEdge_emptyKeyValue() throws {
        XCTAssertEqual(try JSONRepair.repairJson("{\"\": \"value\""), JSONRepair.convertToJSONValue(["": "value"]))
    }
    
    func testEdge_misplacedQuoteInValue() throws {
        // Input: {"key": "v"alue"}  -> Python: {"key": "v\"alue\""}
        let result = try JSONRepair.repairJson("{\"key\": \"v\"alue\"}")
    ...
    func testEdge_commentBetweenPairs() throws {
        XCTAssertEqual(try JSONRepair.repairJson("{\"value_1\": true, COMMENT \"value_2\": \"data\"}"), JSONRepair.convertToJSONValue(["value_1": true, "value_2": "data"]))
    }
    
    func testEdge_junkBetweenPairs() throws {
        XCTAssertEqual(try JSONRepair.repairJson("{\"value_1\": true, SHOULD_NOT_EXIST \"value_2\": \"data\" AAAA }"), JSONRepair.convertToJSONValue(["value_1": true, "value_2": "data"]))
    }
    
    func testEdge_emptyKeyWithSpaceColon() throws {
        // Python: {"" : true, "key2": "value2"} -> {"": true, "key2": "value2"}
        XCTAssertEqual(try JSONRepair.repairJson("{\"\" : true, \"key2\": \"value2\"}"), JSONRepair.convertToJSONValue(["": true, "key2": "value2"]))
    }
    
    func testEdge_unquotedValueWithTrailingQuote() throws {
        // { "words": abcdef", "numbers": 12345", "words2": ghijkl" }
        let result = try JSONRepair.repairJson("{ \"words\": abcdef\", \"numbers\": 12345\", \"words2\": ghijkl\" }")
    ...
    func testEdge_missingCommaDoubledQuote() throws {
        ...
        let result = try JSONRepair.repairJson("{\"number\": 1,\"reason\": \"According...\"\"ans\": \"YES\"}")
    ...
    func testEdge_nestedObjectInStringValue() throws {
        // { "a" : "{ b": {} }" }
        let result = try JSONRepair.repairJson("{ \"a\" : \"{ b\": {} }\" }")
    ...
    func testEdge_trailingBoolAfterString() throws {
        // {"b": "xxxxx" true}
        let result = try JSONRepair.repairJson("{\"b\": \"xxxxx\" true}")
    ...
    func testEdge_loremWithQuotesInValue() throws {
        // {"key": "Lorem "ipsum" s,"}
        let result = try JSONRepair.repairJson("{\"key\": \"Lorem \"ipsum\" s,\"}")
    ...
    func testEdge_unquotedValueWithComma() throws {
        ...
        let result = try JSONRepair.repairJson("{\"lorem\": ipsum, sic, datum.\",}")
    ...
    func testEdge_unquotedValuesMultipleKeys() throws {
        ...
        let result = try JSONRepair.repairJson("{\"lorem\": sic tamet. \"ipsum\": sic tamet, quick brown fox. \"sic\": ipsum}")
    ...
    func testEdge_missingClosingQuote() throws {
        ...
        let result = try JSONRepair.repairJson("{\"lorem_ipsum\": \"sic tamet, quick brown fox. }")
    ...
    func testEdge_unquotedValueCommaSpacedKey() throws {
        ...
        XCTAssertEqual(try JSONRepair.repairJson("{\"key\":value, \" key2\":\"value2\" }"), JSONRepair.convertToJSONValue(["key": "value", " key2": "value2"]))
    }
    ...
    func testEdge_singleQuoteBracesInValue() throws {
        ...
        XCTAssertEqual(try JSONRepair.repairJson("{'text': 'words{words in brackets}more words'}"), JSONRepair.convertToJSONValue(["text": "words{words in brackets}more words"]))
    }
    
    func testEdge_unquotedBracesInValue() throws {
        // {text:words{words in brackets}}
        let result = try JSONRepair.repairJson("{text:words{words in brackets}}")
    ...
    func testEdge_unquotedBracesInValueSuffix() throws {
        ...
        let result = try JSONRepair.repairJson("{text:words{words in brackets}m}")
    ...
    func testEdge_trailingCodeFence() throws {
        // {"key": "value, value2"```
        XCTAssertEqual(try JSONRepair.repairJson("{\"key\": \"value, value2\"```"), JSONRepair.convertToJSONValue(["key": "value, value2"]))
    }
    ...
    func testEdge_emptyKey() throws {
        XCTAssertEqual(try JSONRepair.repairJson("{\"key\": , \"key2\": \"value2\"}"), JSONRepair.convertToJSONValue(["key": "", "key2": "value2"]))
    }
    
    func testEdge_setLikeBraces() throws {
        // {'item1', 'item2', 'item3'}
        XCTAssertEqual(try JSONRepair.repairJson("{'item1', 'item2', 'item3'}"), JSONRepair.convertToJSONValue(["item1", "item2", "item3"]))
    }
}
