...
final class StrictModeTests: XCTestCase {
    ...
    func test_strict_rejects_multiple_top_level_values() {
        XCTAssertThrowsError(try JSONRepair.repairJson("{\"key\":\"value\"}[\"value\"]", strict: true)) { error in
    ...
    func test_strict_duplicate_keys_inside_array() {
        ...
        XCTAssertThrowsError(try JSONRepair.repairJson(payload, strict: true)) { error in
    ...
    func test_strict_requires_colon_between_key_and_value() {
        XCTAssertThrowsError(try JSONRepair.repairJson("{\"missing\" \"colon\"}", strict: true)) { error in
    ...
    func test_strict_rejects_empty_object_with_extra_characters() {
        XCTAssertThrowsError(try JSONRepair.repairJson("{\"dangling\"}", strict: true)) { error in
            XCTAssertTrue("\(error)".contains("Parsed object is empty"))
        }
    }

    func test_strict_rejects_empty_escaped_object_with_extra_characters() {
        XCTAssertThrowsError(try JSONRepair.repairJson("{\\\"key\\\": \\\"value\\\"}", strict: true)) { error in
            XCTAssertTrue("\(error)".contains("Parsed object is empty"))
        }
    }

    func test_strict_detects_immediate_doubled_quotes() {
        XCTAssertThrowsError(try JSONRepair.repairJson("{\"key\": \"\"\"\"}", strict: true)) { error in
